        ...
        {teams.map(team => (
          <div key={team.id} className="mb-2">
            ...
            >
              ...
              <button className="hover:bg-gray-300 p-1 rounded" title="Add channel">
                <Plus size={16} />
              </button>
            </button>
              ...
              <div className="ml-4">
                {team.channels.map(channel => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-2 hover:bg-gray-200 transition-colors ${
                      currentChannel?.id === channel.id ? 'bg-white border-l-4 border-teams-purple' : ''
                    }`}
                  >
                    <div className="flex items-center space-x-2">
                      <Hash size={16} className="text-gray-600" />
                      <span className={currentChannel?.id === channel.id ? 'font-semibold' : ''}>
                        {channel.name}
                      </span>
                    </div>
                    {channel.unread > 0 && (
