...
function Message({ message, user, isCurrentUser }) {
  ...
  return (
    <div className={`flex space-x-3 hover:bg-gray-50 p-2 rounded message-enter ${isCurrentUser ? 'bg-blue-50' : ''}`}>
      <div className="relative">
        <span className="text-3xl">{user.avatar}</span>
        <span className={`status-indicator ${getStatusColor(user.status)} absolute bottom-0 right-0 border-2 border-white`}></span>
      </div>
