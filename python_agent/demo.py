...
2. Intelligent agent with task-specific policies
...
def demo_task_definitions():
    ...
    for i, (task_id, task) in enumerate(tasks.items(), 1):
        ...
        print(f"   Max Steps: {task['maxSteps']} | Completion Reward: +{task['reward']}")
        print()
...
def demo_episode_history():
    ...
    if not history:
        ...
        print(f"{'#':3} {'Task':25} {'Result':8} {'Reward':8} {'Steps':6} {'Duration':10}")
        print('─'*70)
